...
class Node:
    ...
    def __init__(self, p,gui_callback=None):
    ...
    def update_winrate(self, new_value):
        with self._lock:
            self._winrate = new_value
            print(self._winrate)
            with open("winrate_log.txt", "a") as file:  # 'a' mode for appending to the file
                file.write(f"Win Rate: {self._winrate}%\n")
            if self.gui_callback:
                self.gui_callback(self._winrate)
    ...
    def get_best_move(self, resign_threshold):
        ...
        self.update_winrate(self.inverse(child.values / child.visits))
